'''
@Author: WANG Maonan
@Date: 2021-08-30 06:22:26
@Description: 将两个·1的 bar height 变为虚线
@LastEditTime: 2021-08-30 09:59:30
'''
import copy
import numpy as np
np.random.seed(777)
...
def dashed_random_double_bar_height_keep_BarTop(im, height_list, dots_positions, vertical_index=None):
    """随机虚线两个竖着的 bar line
    
    Args:
        im (array): bar chart 数据
        height_list (array): bar chart 的每一个 bar height
        vertical_index (list): 哪一种模式, 形如 [0]
    """
    ...
    if vertical_index == None:
        vertical_index = np.random.choice(list(range(4)), 1, replace=False) # 要删除的一个 index, 从 4 个里面选择 2 个, 共有 4 种可能
    if vertical_index[0] == 0:
        for _i in range(1, 100, 2):
            image[100-_i, left_bar_positions[0]] = 0 # 左侧 bar 的 left line
            image[100-_i, right_bar_positions[0]] = 0
    elif vertical_index[0] == 1:
        for _i in range(1, 100, 2):
            image[100-_i, left_bar_positions[0]] = 0
            image[100-_i, right_bar_positions[1]] = 0
    elif vertical_index[0] == 2:
        for _i in range(1, 100, 2):
            image[100-_i, left_bar_positions[1]] = 0
            image[100-_i, right_bar_positions[0]] = 0
...
def change_numpy2dashedRandomDoubleVerticalLineKeepBarTop(
    ...
    for image_index, image in enumerate(image_numpy_data):
        new_image = dashed_random_double_bar_height_keep_BarTop(
                                            im = image[0], 
    ...
    logger.info('将原始数据集修改为 Dashed Random Double Bar Height but Keep Bar Top.')
