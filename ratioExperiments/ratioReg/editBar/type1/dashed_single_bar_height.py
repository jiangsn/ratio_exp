...
@Description: 一共有 4 个 vertical line, 随机虚线一个, 但是保留 BarTop
...
def dashed_random_single_bar_height_keep_BarTop(im, height_list, dots_positions, vertical_index=None):
    """随机虚线一个竖着的 bar line, 但是会保留 BaoTop
...
def change_numpy2dashedRandomSingleBarHeightKeepBarTop(
                                image_numpy_file:str,
                                label_numpy_file:str,
                                bar_height_numpy_file:str, 
                                dots_positions_numpy_file:str
                            ):
    """将原始的 image 转换为「去除一个 bar height 但是保留 barTop」
    """
    ...
    for image_index, image in enumerate(image_numpy_data):
        new_image = dashed_random_single_bar_height_keep_BarTop(
                                            im = image[0], 
    ...
    logger.info('将原始数据集修改为 Remove Random Single Bar Heights but Keep BarTop.')
