...
@Description: 修改 bar, 只保留左侧的一个 bar-height
@LastEditTime: 2021-08-16 13:38:01
'''
import sys
...
def keep_left_bar_height(im, height_list, dots_positions):
    """只保留左侧的竖着的 bar, 其余的都删除
    ...
    # 去除 right bar
    image[:, left_bar_positions[1]] = 0 # 左侧 bar 的 right-bar-height
    image[:, right_bar_positions[1]] = 0 # 右侧 bar 的 right-bar-height

    return image


def change_numpy2keepleftbarHeight(image_numpy_file, height_numpy_file, dots_positions_numpy_file):
    ...
    for image_index, image in enumerate(image_numpy_data):
        new_image = keep_left_bar_height(im = image[0], 
...
if __name__ == "__main__":
    ...
    image_tensor = change_numpy2keepleftbarHeight(image_numpy_file, height_numpy_file, dots_positions_numpy_file)

    # 将 tensor 转换为 np.array
    save_path = './ratioRegression/editBar/example/keep_left_barHeight'
