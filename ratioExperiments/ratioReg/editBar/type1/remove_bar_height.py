...
def remove_bar_height(im, height_list, dots_positions):
    """将垂直线删除
    Args:
    ...
    # 移除 bar line
    for dot_index in [left_dot_index, right_dot_index]: # 依次获得 left-dot-index 和 right-dot-index
        (left_bar_position, right_bar_position) = line_index2position[dot_index]
        image[:, left_bar_position] = 0
        image[:, right_bar_position] = 0
...
def change_numpy2removeBarHeight(
    ...
    for image_index, image in enumerate(image_numpy_data):
        new_image = remove_bar_height(im = image[0], 
    ...
    logger.info('将原始数据集修改为 remove-vertical-lines.')
