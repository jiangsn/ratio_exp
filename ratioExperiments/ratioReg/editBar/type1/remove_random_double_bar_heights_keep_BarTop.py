...
@Description: 一共有两个 bar, 我们随机从两个 bar 中各删除一条线
...
def remove_random_double_bar_height_keep_BarTop(im, height_list, dots_positions, vertical_index=None):
    """随机删除两个竖着的 bar line
...
def change_numpy2removeRandomDoubleVerticalLineKeepBarTop(
    ...
    for image_index, image in enumerate(image_numpy_data):
        new_image = remove_random_double_bar_height_keep_BarTop(
                                            im = image[0], 
    ...
    logger.info('将原始数据集修改为 Remove Random Double Bar Height but Keep Bar Top.')
