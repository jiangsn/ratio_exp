...
@Description: 移除垂直线的中间部分
@LastEditTime: 2021-08-16 12:43:37
'''
import sys
...
def remove_vertiacl_middle_line(im, height_list, dots_positions, height_minus=None, bar_index=None):
    """移除垂直的线的中间部分
    ...
    # 随机选择一个 bar, 计算减少多少高度
    if bar_index == None: # 是否指定了 selected bar index
    ...
    if height_minus == None: # 是否指定了减少的长度
        if height_list[selected_bar_index]>=6:
            minus = np.random.choice(list(range(0, int(height_list[selected_bar_index]-5)))) # 随机需要减少的长度, 最短需要保留 5 个长度
        else:
            minus = 0
    else:
        minus = height_minus
    ...
    down_position = int(100 - height_list[selected_bar_index]/2 + minus/2)
    image[down_position-minus:down_position, selected_bar_position[0]] = 0
    image[down_position-minus:down_position, selected_bar_position[1]] = 0

    height_list[selected_bar_index] = (height_list[selected_bar_index]-minus)/2 # 新的 bar height, 这里只算半截的长度
...
def change_numpy2removeVerticalMiddleLines(image_numpy_file, height_numpy_file, dots_positions_numpy_file):
    ...
    for image_index, image in enumerate(image_numpy_data):
        new_image, new_ratio = remove_vertiacl_middle_line(im = image[0], 
...
if __name__ == "__main__":
    ...
    image_tensor, label_tensor = change_numpy2removeVerticalMiddleLines(image_numpy_file, height_numpy_file, dots_positions_numpy_file)

    # 将 tensor 转换为 np.array
    save_path = './ratioRegression/editBar/example/remove_vertical_middle_line'
