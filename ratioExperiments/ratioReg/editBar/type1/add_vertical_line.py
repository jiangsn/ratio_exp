...
@Description: 增长较短的 bar
@LastEditTime: 2021-07-20 04:59:20
'''
import sys
...
def add_vertiacl_line(im, height_list, dots_positions, height_minus=None, bar_index=None):
    ...
    # 删除两个 bar 的 bar top
    left_bar_positions = line_index2position[left_dot_index] # 获得左侧 bar 的两个竖线的坐标, 例如 (11, 18)
    right_bar_positions = line_index2position[right_dot_index]
    image[99-left_bar_height, left_bar_positions[0]+1:left_bar_positions[1]] = 0 # 去除第一个水平线
    image[99-right_bar_height, right_bar_positions[0]+1:right_bar_positions[1]] = 0 # 去除第二个水平线
    ...
    # 增加他的高度
    selected_bar_position = line_index2position[selected_bar_index]
...
def change_numpy2addVerticalLines(image_numpy_file, height_numpy_file, dots_positions_numpy_file, height_minus_list=None, bar_index_list=None):
    ...
    for image_index, image in enumerate(image_numpy_data):
        if (height_minus_list == None) and (bar_index_list == None):
            new_image, new_ratio = add_vertiacl_line(im=image[0], 
...
if __name__ == "__main__":
    ...
    image_tensor, label_tensor = change_numpy2addVerticalLines(image_numpy_file, height_numpy_file, dots_positions_numpy_file)

    # 将 tensor 转换为 np.array
    save_path = './ratioRegression/editBar/example/add_vertical_line'
