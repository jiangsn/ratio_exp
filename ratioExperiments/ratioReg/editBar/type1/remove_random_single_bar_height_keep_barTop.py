...
@Description: 一共有 4 个 vertical line, 随机删除一个, 但是保留 BarTop
...
def remove_random_single_bar_height_keep_BarTop(im, height_list, dots_positions, vertical_index=None):
    """随机删除一个竖着的 bar line, 但是会保留 BaoTop
...
def change_numpy2removeRandomSingleBarHeightKeepBarTop(
    ...
    for image_index, image in enumerate(image_numpy_data):
        new_image = remove_random_single_bar_height_keep_BarTop(
                                            im = image[0], 
