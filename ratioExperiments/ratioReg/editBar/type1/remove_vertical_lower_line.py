'''
@Author: WANG Maonan
@Date: 2021-06-17 16:13:52
@Description: 随机减少某个 bar 的长度
@LastEditTime: 2021-08-16 12:43:13
'''
import sys
...
def remove_vertiacl_lower_line(im, height_list, dots_positions, height_minus=None, bar_index=None):
    """移除垂直的线的上半部分
    ...
    if height_minus == None: # 是否指定了减少的长度
        if height_list[selected_bar_index]>=6: # 如果 bar height 太短, 则不需要再进行裁剪
    ...
    image[100-minus:100, selected_bar_position[0]] = 0
    image[100-minus:100, selected_bar_position[1]] = 0
...
def change_numpy2removeVerticalLowerLines(image_numpy_file, height_numpy_file, dots_positions_numpy_file):
    ...
    for image_index, image in enumerate(image_numpy_data):
        new_image, new_ratio = remove_vertiacl_lower_line(im = image[0], 
    ...
    logger.info('将原始数据集修改为 remove lower vertical lines.')
...
if __name__ == "__main__":
    ...
    image_tensor, label_tensor = change_numpy2removeVerticalLowerLines(image_numpy_file, height_numpy_file, dots_positions_numpy_file)

    # 将 tensor 转换为 np.array
    save_path = './ratioRegression/editBar/example/remove_vertical_lower_line'
